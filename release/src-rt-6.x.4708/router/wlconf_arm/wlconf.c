...
wlconf_ampdu_amsdu_set(char *name, char prefix[PREFIX_LEN], int nmode, int btc_mode, int ap)
{
	...
	foreach(var, caps, next) {
		...
		if (amsdu) {
			...
			nvram_str = nvram_get(strlcat_r(prefix, "rx_amsdu_in_ampdu", buf, sizeof(buf)));
			if (nvram_str) {
				if (!strcmp(nvram_str, "on"))
...
//	restore_defaults = !nvram_get(strlcat_r(prefix, "ifname", tmp, sizeof(tmp)));
	...
	*/
	...
	str = nvram_get(strlcat_r(prefix, "frameburst_dyn_max_stations", tmp, sizeof(tmp)));
	...
	val = 0;
	str = nvram_get(strlcat_r(prefix, "taf_rule", tmp, sizeof(tmp)));
	if (str) {
	...
	val = 0;
	str = nvram_get(strlcat_r(prefix, "ebos_enable", tmp, sizeof(tmp)));
	if (str) {
		val = atoi(str);
	}
	...
	val = 0;
	str = nvram_get(strlcat_r(prefix, "ebos_flags", tmp, sizeof(tmp)));
	if (str) {
		val = strtoul(str, NULL, 0);
	...
	val = 0;
	str = nvram_get(strlcat_r(prefix, "ebos_prr_threshold", tmp, sizeof(tmp)));
	if (str) {
		val = strtoul(str, NULL, 0);
	...
	val = 0;
	str = nvram_get(strlcat_r(prefix, "ebos_prr_flags", tmp, sizeof(tmp)));
	if (str) {
		val = strtoul(str, NULL, 0);
	...
	val = 0;
	str = nvram_get(strlcat_r(prefix, "ebos_prr_transit", tmp, sizeof(tmp)));
	if (str) {
	...
	str = nvram_get(strlcat_r(prefix, "ebos_transit", tmp, sizeof(tmp)));
	if (str) {
		val = strtol(str, NULL, 0);
	...
	val = 0;
	if (ap) {
		str = nvram_get(strlcat_r(prefix, "pspretend_threshold", tmp, sizeof(tmp)));
		if (str) {
			val = atoi(str);
		}
	}
