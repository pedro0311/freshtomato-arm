...
static int bsd_proc_event(bsd_info_t*info, uint8 remote, char *pkt, int bytes)
{
	...
	if(info->is_re)
		ifname = convert_apif(ifname);
	BSD_EVENT("chk apif(%d):->%s\n", info->is_re, ifname);
	if(!ifname)
		BSD_ERROR("\n!! invalid apif translation\n");
	...
	switch (evt_type) {
		...
		case WLC_E_DISASSOC_IND:
			...
			break;
		case WLC_E_AUTH_IND:
			BSD_INFO("WLC_E_AUTH_IND...from "MACF"\n", ETHERP_TO_MACF(addr));
			...
			if(info->amas_sta != (void*)-1) {
				bsd_chk_amas_sta(info, NULL, ifname, addr, TRUE);
				bsd_dump_amas(info);
				update_adbnv(info);
			}
#endif
		...
		case WLC_E_PROBREQ_MSG:
			BSD_INFO("Probe-req...from if:%s "MACF"\n", ifname, ETHERP_TO_MACF(addr));
			bsd_add_prbsta(info, ifname,  remote, addr);
#ifdef AMASDB
			if(info->amas_sta != (void*)-1) {
				bsd_chk_amas_sta(info, NULL, ifname, addr, info->amasdb_from_probe);
				bsd_dump_amas(info);
				update_adbnv(info);
			}
#endif
...
int bsd_proc_socket2(bsd_info_t*info, struct timeval *tv, char *ifreq, uint8 token,
	struct ether_addr *bssid)
{
	fd_set fdset;
	int fdmax;
	int width, status = 0, bytes;
	char buf_ptr[BSD_BUFSIZE_4K], *pkt = buf_ptr;
	...
	/* build file descriptor set now to save time later */
	if (info->event_fd2 != BSD_DFLT_FD) {
		FD_SET(info->event_fd2, &fdset);
		fdmax = info->event_fd2;
	}
	...
	/* handle brcm event */
	if (info->event_fd2 !=  BSD_DFLT_FD && FD_ISSET(info->event_fd2, &fdset)) {

		memset(pkt, 0, sizeof(buf_ptr));
		if ((bytes = recv(info->event_fd2, pkt, sizeof(buf_ptr), 0)) <= IFNAMSIZ) {
			BSD_EVENT("BSS Transit Response: recv err\n");
			return BSD_OK;
		}
		...
		if (pdata_len <= BCM_EVENT_HEADER_LEN)
			BSD_EVENT("BSS Transit Response: data_len %d too small\n", pdata_len);
		...
		if (strncmp(ifname, ifreq, strlen(ifreq)) != 0) {
			...
			BSD_EVENT("BSS Transit Response: not for interface %s\n", ifreq);
			return BSD_OK;
		}
		...
		if (bsstrans_resp->token != token) {
			/* not for the requested interface */
			BSD_EVENT("BSS Transit Response: not for token %x\n", token);
			return BSD_OK;
		}

		/* reject */
		if (bsstrans_resp->status) {
			BSD_EVENT("BSS Transit Response: STA reject\n");
			return BSD_FAIL;
		}

		/* accept, but use another target bssid */
		if (eacmp(bssid, addr) != 0) {
			BSD_EVENT("BSS Transit Response: target bssid not same\n");
			return BSD_FAIL;
		}
	}
