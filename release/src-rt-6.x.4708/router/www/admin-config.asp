...
<div class="section">
	<div>
		<script>
			W('<input type="text" size="60" maxlength="128" id="backup-name" onchange="backupNameChanged()" value="FreshTomato_'+('<% version(); %>'.replace(/\./g, '_'))+'~m'+nvram.et0macaddr.replace(/:/g, '').substring(6, 12)+'~'+nvram.t_model_name.replace(/\/| /g, '_')+'~'+now.getFullYear()+('0'+(now.getMonth()+1)).slice(-2)+('0'+(now.getDate()+1)).slice(-2)+'">');
		</script>
