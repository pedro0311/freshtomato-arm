...
for (i = 0; i < tabs.length; ++i)
{
	...
	createFieldTable('', [
		...
		{ title: '<div title="Accept options pushed by server EXCEPT for routes, block-outside-dns and dhcp options like DNS servers.">Ignore Redirect Gateway (route-nopull)<\/div>', name: 'f_vpn_'+t+'_nopull', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_nopull' ) != 0 },
		{ title: '<div title="Don\'t add or remove routes automatically (no DNS leaks). ">Ignore Redirect Gateway (route-noexec)<\/div>', name: 'f_vpn_'+t+'_noexec', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_noexec' ) != 0 },
