...
void start_bittorrent(void)
{
	...
	fprintf(fp,
		 ...
		 */
		"sed -i 's/,,\\s/, /g' /tmp/settings.json\n"
		"mv /tmp/settings.json %s/.settings\n"
...
void stop_bittorrent(void)
{
	...
	fprintf(fp,
		...
		"fi\n"
		"/usr/bin/btcheck addcru\n"
		"echo 120832 > /proc/sys/net/core/rmem_max\n"
		"echo 120832 > /proc/sys/net/core/wmem_max\n"
		"exit 0\n");
	fclose(fp);
