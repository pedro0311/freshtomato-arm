...
struct ipv6_stub_type {
	...
	int (*ipv6_dst_lookup)(struct sock *sk, struct dst_entry **dst, struct flowi *fl);
};
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0) && !defined(ISRHEL7) && !defined(CONFIG_ARM)
#include <net/genetlink.h>
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 17, 0) && !defined(ISRHEL7)
#define sw_hash ignore_df = 0; skb->nf_trace
#endif
