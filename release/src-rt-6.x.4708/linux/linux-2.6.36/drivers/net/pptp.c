...
#include <net/icmp.h>
#include <net/route.h>
#ifdef CONFIG_NET_IPGRE_DEMUX_MODULE
#include <net/gre.h>
#endif
...
static int pptp_rcv(struct sk_buff *skb)
{
...
#ifndef CONFIG_NET_IPGRE_DEMUX_MODULE
	...
	if (ntohs(header->protocol) != PPTP_GRE_PROTO || /* PPTP-GRE protocol for PPTP */
		((header->ver & 0x7F) != PPTP_GRE_VER) || /* version should be 1 */
...
#else
static const struct net_protocol net_pptp_protocol = {
	.handler = pptp_rcv,
};
#endif
...
#else
	err = inet_add_protocol(&net_pptp_protocol, IPPROTO_GRE);
	if (err) {
		printk(KERN_INFO "PPTP: can't add protocol\n");
		goto out_mem_free;
	}
